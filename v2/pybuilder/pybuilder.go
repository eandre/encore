...
func getPyParserPath() (string, error) {
	const pyParserBinaryName = "pyparser-encore"

	if path := os.Getenv("ENCORE_PYPARSER_PATH"); path != "" {
		return path, nil
	}

	// Check the encore bin directory.
	if bin, ok := env.EncoreBin().Get(); ok {
		candidate := filepath.Join(bin, pyParserBinaryName)
...
func (i *BuilderImpl) Parse(ctx context.Context, p builder.ParseParams) (*builder.ParseResult, error) {
	...
	cmd.Env = append(cmd.Env,
		"ENCORE_PY_RUNTIME_PATH="+pyRuntimePath.ToIO(),
...
func (i *BuilderImpl) RunTests(ctx context.Context, p builder.RunTestsParams) error {
	// Python tests are run using pytest or similar
...
func (i *BuilderImpl) TestSpec(ctx context.Context, p builder.TestSpecParams) (*builder.TestSpecResult, error) {
	...
	command := res.Cmd.Command.Expand("")
	args := append(command[1:], p.Args...)

	envs := append(os.Environ(), p.Env...)
	envs = append(envs, res.Cmd.Env.Expand("")...)
...
// Reports the Python Runtime root directory.
func pyRuntimeRoot(runtimesPath paths.FS) paths.FS {
	return runtimesPath.Join("py")
}
