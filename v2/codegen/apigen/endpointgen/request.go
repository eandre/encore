...
func (d *requestDesc) EncodeExternalReq() *Statement {
	return Func().Params(
		...
		d.renderRequestEncoding(g)
		g.Return(d.httpHeaderExpr(), d.queryStringExpr(), Err())
	})
}

func (d *requestDesc) renderRequestEncoding(g *Group) {
	if d.ep.Request == nil {
		return
	}

	enc := d.ep.RequestEncoding()[0]
	apigenutil.EncodeHeaders(d.gu.Errs, g, d.httpHeaderExpr(), d.reqDataPayloadExpr(), enc.HeaderParameters)
	apigenutil.EncodeQuery(d.gu.Errs, g, d.queryStringExpr(), d.reqDataPayloadExpr(), enc.QueryParameters)
	apigenutil.EncodeBody(g, d.jsonStream(), d.reqDataPayloadExpr(), enc.BodyParameters)
}
