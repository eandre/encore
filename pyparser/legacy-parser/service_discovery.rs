...
pub fn discover_services(
    ...
    for bind in binds {
        let range = bind.range;
        if !range.is_valid() {
            continue;
        }

        let Some(file) = range.file_id.and_then(|id| file_set.lookup_file(id)) else {
            continue;
        };
...
fn merge_roots(roots: Vec<ServiceRoot>) -> Vec<DiscoveredService> {
    ...
    for root in selected {
        ...
        let is_descendant = final_roots.iter().any(|parent| {
            root.path.starts_with(&parent.path) && root.path != parent.path
        });
