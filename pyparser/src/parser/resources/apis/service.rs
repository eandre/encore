...
fn parse_services(ctx: &mut ResourceParseContext) -> ParseResult<()> {
    // Collect assignments first to avoid borrow issues
    let assignments: Vec<_> = ctx.module_obj.assignments().to_vec();
    ...
    for assignment in assignments {
        ...
        let service = Service {
            name: name.clone(),
            span: ctx.range(assignment.range.to_text_range()),
            rel_path,
            doc: None,
        };
